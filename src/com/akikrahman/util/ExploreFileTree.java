...
public class ExploreFileTree implements FileVisitor<Path> {
	...
	public FileVisitResult visitFile(Path file, BasicFileAttributes attr) {
		...
		/*
		 * Useful debug code System.out.print("visitFile: ");
		 * 
		 * if (attr.isSymbolicLink()) { System.out.format("Symbolic link: %s ", file); }
		 * else if (attr.isRegularFile()) { System.out.format("Regular file: %s ",
		 * file); } else { System.out.format("Other: %s ", file); }
		 * System.out.println("(" + attr.lastModifiedTime() + " modified time "+
		 * attr.size() + " bytes)");
		 */
		...
		try {
			...
			if (strFile.contains(".")) {
				...
				if (extension.length() > 20)
					extension = ""; // an invalid extension found, more than 20 chars
			}
			...
			if (strFile.contains("\\") && strFile.contains(".")) {
				...
				if(start<end) name = strFile.substring( start, end);
			...
			String filePath = "";
			if (strFile.contains("\\")) {
				filePath = strFile.substring(0, strFile.lastIndexOf('\\'));
			}
